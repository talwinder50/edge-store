...
func TestStartCmdContents(t *testing.T) {
	...
	require.Equal(t, "Start edge-auth", startCmd.Short)
	require.Equal(t, "Start edge-auth", startCmd.Long)
...
func TestStartCmdWithMissingHostArg(t *testing.T) {
	...
	require.Equal(t,
		"Neither host-url (command line flag) nor EDGE-AUTH_HOST_URL (environment variable) have been set.",
		err.Error())
}

func TestStartEdgeAuthWithBlankHost(t *testing.T) {
	parameters := &edgeAuthParameters{hostURL: ""}

	err := startEdgeAuth(parameters)
